...
function Evaluate({ showGraph }) {
  ...
  const fetchSubmissions = async () => {
    try {
      const { data: ecopoints, error: ecoError } = await supabase
        .from("ecopoints")
        .select("*");
      ...
      const submissions = await Promise.all(
        ecopoints.map(async (r) => {
          const binLookupId = parseInt(r.smartbinno, 10);

          if (isNaN(binLookupId)) {
            console.error("Invalid bin ID in ecopoints:", r.smartbinno);
            return null;
          }

          const { data: bin, error: binError } = await supabase
            .from("smartbin")
            .select("latitude, longitude, wastetype")
            .eq("binid", binLookupId)
            .single();

          if (binError || !bin) return null;
          ...
          return {
            ...
            wasteType: bin.wastetype ? bin.wastetype.toLowerCase() : "other",
          };
        }),
      );
  ...
  useEffect(() => {
    if (councilRoutes && councilRoutes.length > 0) {
  ...
  const fetchTrees = async () => {
    try {
      const { data: treeRequests, error } = await supabase
        .from("treecompleted")
        .select("*")
        .gte("treelevel", 99.9)
        .lte("treelevel", 100.1);
      ...
      const filteredTrees = await Promise.all(
        treeRequests.map(async (t) => {
          try {
            const { data: citizenTree, error: citizenError } = await supabase
              .from("citizen")
              .select("nearestcouncil")
              .eq("citizenid", t.citizenno)
              .single();
            ...
            return {
              ...t,
              dateT: t.completeddate,
              nearestcouncil: citizenTree.nearestcouncil,
            };
          } catch (citErr) {
      ...
      setTreeData(
        filteredTrees.filter((t) => t && t.nearestcouncil === councilName),
      );
    } catch (err) {
  ...
  const pageWidth = doc.internal.pageSize.getWidth();
   ...
   doc.addImage(
    ...
    autoTable(doc, {
      ...
      startY: 55,
      theme: "grid",
      headStyles: {   fillColor: [30, 161, 52], textColor: [255, 255, 255],  lineColor: [3, 88, 18]     },
    });
